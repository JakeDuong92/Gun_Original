...
{	
	public class BotProperties : MonoBehaviour 
	{
		...
		public void SetParentHead (GameObject objValue) {
			...
			if (objHead) {
				var objBotHeadTrigger = objHead.GetComponent<BotHeadTrigger> ();
				if (objBotHeadTrigger) {
					objBotHeadTrigger.SetBotController (objValue);
				}
			}
		}
		...
		public void ReceiveImpactPlayer (Transform transformImpact, float numImpact) {
			...
			if (countLife == 0.0f) {
				...
				if (objParentPlayer) {
					var objBotController = objParentPlayer.GetComponent<BotController> ();
					if (objHead) {
			...
			if (string.Equals (otherobjs.gameObject.tag, GlobalEnvironment.idBulletTag)) {
				...
				if (!string.Equals (retTag, GlobalEnvironment.idBotTag)) {
					...
					case GlobalEnvironment.idMachineGunWeapon:
						impactLevel = 2;
						if (bulletWeapon.GetImpactMachineGun () == 2) {
							Destroy (otherobjs.gameObject);
						} else {
							bulletWeapon.PlusImpactMachineGun ();
						}
						break;
